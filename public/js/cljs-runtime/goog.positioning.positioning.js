...
goog.positioning.positionAtAnchor = function(anchorElement, anchorElementCorner, movableElement, movableElementCorner, opt_offset, opt_margin, opt_overflow, opt_preferredSize, opt_viewport) {
  ...
  if (corner & goog.positioning.CornerBit.RIGHT) {
    offsetLeft = offsetLeft + anchorRect.width;
  } else if (corner & goog.positioning.CornerBit.CENTER) {
    offsetLeft = offsetLeft + anchorRect.width / 2;
  }
...
goog.positioning.adjustForViewport_ = function(pos, size, viewport, overflow) {
  ...
  if (overflow & goog.positioning.Overflow.FAIL_X) {
    status = status | ((pos.x < viewport.left ? goog.positioning.OverflowStatus.FAILED_LEFT : 0) | (pos.x + size.width > viewport.right ? goog.positioning.OverflowStatus.FAILED_RIGHT : 0));
  }
  if (pos.y < viewport.top && overflow & goog.positioning.Overflow.ADJUST_Y) {
    pos.y = viewport.top;
    status = status | goog.positioning.OverflowStatus.ADJUSTED_Y;
  }
  if (overflow & goog.positioning.Overflow.RESIZE_HEIGHT) {
    var originalY = pos.y;
    if (pos.y < viewport.top) {
      pos.y = viewport.top;
      status = status | goog.positioning.OverflowStatus.HEIGHT_ADJUSTED;
    }
  ...
  if (overflow & goog.positioning.Overflow.FAIL_Y) {
    status = status | ((pos.y < viewport.top ? goog.positioning.OverflowStatus.FAILED_TOP : 0) | (pos.y + size.height > viewport.bottom ? goog.positioning.OverflowStatus.FAILED_BOTTOM : 0));
  }
  return status;
};
